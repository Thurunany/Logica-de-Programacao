...
def preco_com_desconto(preco_original, percentual_desconto):
    ...
    preco = preco_original * percentual_desconto/100
    novo_preco = preco_original - preco
...
def dias_perdidos_por_fumar(cigarros_fumados_por_dia, anos_fumando):
    ...
    dias = cigarros_fumados_por_dia * 365 * anos_fumando * 10 / 1440
    dias = round(dias, 2)
    return dias
...
def salario(valor_hora, horas_mensais):
    ...
    salB = valor_hora * horas_mensais
    desconto = salB * 24 / 100
    salL =  salB - desconto
    return salL
...
def maximo(lista):
    ...
    return max.lista(1, key=float)

def minimo(lista):
    """ Calcule o menor número da 'lista' """
    return min.lista(1, key=float)
